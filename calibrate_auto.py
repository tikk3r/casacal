...
flux_calibrator_band = 'Q'
myconfig = 'C'                                                      # VLA array configuration used for the observation.
myband = 'Q'                                                        # Observing band.

# MS specific
msfile = '10B-245.sb2198697.eb2635906.55529.962145833335.ms'        # Original MS file.
...
myopacspw = '0~1'                                                   # Spectral windows to calculate opacities for (again NEW indices (i.e. 0,1...)).
mssplit = '10B-245.srcs_Q.ms'                                       # MS file with the interesting sources from `myfield`.
msscans = '10B-245.srcs_Q.ms.txt'                                   # Text file to write `listobs` output to.
mstarget = 'target_Q.ms'                                            # MS file of the target.
myimage = 'target_Q_image'                                          # Image.
mycube = 'target_Q_cube'                                            # Cube.
mycontsub = 'target_Q.ms.contsub'                                   # Continuum subtraction.
skymodel = 'target_Q.model'                                         # Sky model.
mymask = 'target_Q.mask'                                            # Mask.
myrefant = 'ea05'                                                   # Reference antenna.

# Spectral line and imaging parameters.
myrestfreq = "46.76607GHz"                                          # Spectral line redshifted rest frequency.
myphasecenter = 'J2000 18h42m22.32 +59d38m29.6'
mylinespw = '1'                                                     # spw containing the line.
mylinechannels = '20~40'                                            # Channels in the selected spw containing the line.
myfitorder = 1
mylinechannels_spectrum = '845~875'                                 # Range of channels containing the line in the full spectrum.
...
print 'Field of view [arcsec]: ', myfov
...
if (mystep in thesteps):
    ...
    gencal(vis=mssplit, caltable="opacity.cal", caltype="opac", spw=myopacspw,antenna="", pol="", parameter=myTau)
...
if (mystep in thesteps):
    if DEBUG:
        print 'SETJY using model source %s_%s.im' % (flux_calibrator, flux_calibrator_band)
    casalog.post('Step '+str(mystep)+' '+step_title[mystep],'INFO')
    print 'Step ', mystep, step_title[mystep]
    # Set the flux scale for field 2 (the flux calibrator) using the model.
    setjy(vis=mssplit,field=myfluxref,spw="",selectdata=False,timerange="",scan="",intent="",observation="",scalebychan=True,standard="Perley-Butler 2010",model='%s_%s.im'%(flux_calibrator,flux_calibrator_band),modimage=None,listmodels=False,fluxdensity=-1,spix=0.0,reffreq="1GHz",polindex=[],rotmeas=0.0,fluxdict={},useephemdir=False,interpolation="nearest",usescratch=True,ismms=None)
# End of step 3.                               #
################################################
...
if(mystep in thesteps):
    ...
    gaincal(vis=mssplit,caltable="delays2.cal",field=myphaseref,spw="*:4~60",intent="",selectdata=True,timerange="",uvrange="",antenna="",scan="",observation="",msselect="",solint="inf",combine="scan",preavg=-1.0,refant=myrefant,minblperant=4,minsnr=3.0,solnorm=False,gaintype="K",smodel=[],calmode="p",append=False,splinetime=3600.0,npointaver=3,phasewrap=180.0,docallib=False,callib="",gaintable=gtables2,gainfield=[],interp=[],spwmap=[],parang=False)
...
if(mystep in thesteps):
    if DEBUG:
        ...
        print 'APPLYCAL using the following interpolation for all fields: ' + ','.join(gfields+['nearest', 'nearest', 'linear', 'nearest'])
    casalog.post('Step '+str(mystep)+' '+step_title[mystep],'INFO')
    print 'Step ', mystep, step_title[mystep]
    # Apply calibrations to the flux calibrator.
    applycal(vis=mssplit,field=myfluxref,spw="",intent="",selectdata=True,timerange="",uvrange="",antenna="",scan="",observation="",msselect="",docallib=False,callib="",gaintable=gtables,gainfield=gfields+[myfluxref, myfluxref, myfluxref, myfluxref],interp=gfields+['nearest', 'nearest', 'linear', 'nearest'],spwmap=[],calwt=False,parang=False,applymode="",flagbackup=True)

    # Apply calibrations to the phase reference.
    applycal(vis=mssplit,field=myphaseref,spw="",intent="",selectdata=True,timerange="",uvrange="",antenna="",scan="",observation="",msselect="",docallib=False,callib="",gaintable=gtables2,gainfield=gfields+[myphaseref, myphaseref, myphaseref, myphaseref],interp=gfields+['nearest', 'nearest', 'linear', 'nearest'],spwmap=[],calwt=False,parang=False,applymode="",flagbackup=True)
...
if(mystep in thesteps):
    casalog.post('Step '+str(mystep)+' '+step_title[mystep],'INFO')
    print 'Step ', mystep, step_title[mystep]
    # Average the data to 30s.
    split(vis=mssplit,outputvis=mstarget,keepmms=True,field=mytarget,spw="",scan="",antenna="",correlation="",timerange="",intent="",array="",uvrange="",observation="",feed="",datacolumn="corrected",keepflags=True,width=1,timebin="",combine="")
    split(vis=mssplit,outputvis=mstarget[:-3]+'_30s.ms',keepmms=True,field=mytarget,spw="",scan="",antenna="",correlation="",timerange="",intent="",array="",uvrange="",observation="",feed="",datacolumn="corrected",keepflags=True,width=1,timebin="30s",combine="scan")
